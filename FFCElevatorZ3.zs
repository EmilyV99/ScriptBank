        ...
        while(true)
        {
            ...
            if(go)
            {
                ...
                while(this->Vy)
                {
                    ...
                    Screen->FastCombo(1, btnX-Viewport->X, this->Y-16-Viewport->Y,BTN_CMB,this->CSet);
                    Waitframe();
                }
            ...
            Waitdraw();
            if(btn)
                Screen->FastCombo(SPLAYER_PLAYER_DRAW, Hero->X+PROMPT_XOFF-Viewport->X, Hero->Y+PROMPT_YOFF-Viewport->Y, PROMPT_CMB, PROMPT_CSET);
            //should use SPLAYER_FFC_DRAW, once that's added
            Screen->FastCombo(1, btnX-Viewport->X, this->Y-16-Viewport->Y,BTN_CMB,this->CSet);
            Waitframe();
        }
